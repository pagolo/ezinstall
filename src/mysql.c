...
void MySqlForm(void) {
   ...
   conn = mysql_init(NULL);
   if (!(conn)) Error(getstr(S_MYSQL_INIT,"error initiating MySQL"));

   if (mysql_real_connect(conn,globaldata.gd_mysql->host,globaldata.gd_mysql->username,globaldata.gd_mysql->password,NULL,0,NULL,0)==NULL) {
	mysql_close(conn);
	Error(getstr(S_MYSQL_CONNECT,"error connecting to MySQL"));
	}
...
void CreateDbTables(void) {
   ...
   if (mysql_select_db(conn,globaldata.gd_mysql->db_name)!=0) {
      ...
      if (mysql_query(conn,query)!=0) {
	mysql_close(conn);
	Error(getstr(S_NO_CREATE_DB,"can't create new database, please go back and select an existing db"));
      } else {
        printf(getstr(S_DB_CREATED,"Database '%s' has been created<br>"),globaldata.gd_mysql->db_name);
      }
      free(query);
    } else {
      printf(getstr(S_DB_FOUND,"Database '%s' was found<br>"),globaldata.gd_mysql->db_name);
    }
   mysql_close(conn);
