...
pkgdatadir = $(datadir)/ezinstall
pkgincludedir = $(includedir)/ezinstall
pkglibdir = $(libdir)/ezinstall
pkglibexecdir = $(libexecdir)/ezinstall
...
DIST_COMMON = INSTALL NEWS README AUTHORS ChangeLog \
	...
	$(srcdir)/config.h.in mkinstalldirs COPYING config.guess \
...
LDFLAGS = 
LIBOBJS = 
LIBS = -lcurl -lbz2 -lz -lpthread -lxml2 -lmysqlclient 
LTLIBOBJS = 
MAINT = #
MAKEINFO = makeinfo
...
PACKAGE_STRING = ezinstall 0.0.25
PACKAGE_TARNAME = ezinstall
PACKAGE_URL = 
...
abs_builddir = /home/paolo/Src/ezinstall-remote
abs_srcdir = /home/paolo/Src/ezinstall-remote
abs_top_builddir = /home/paolo/Src/ezinstall-remote
abs_top_srcdir = /home/paolo/Src/ezinstall-remote
ac_ct_CC = gcc
am__include = include
am__leading_dot = .
am__quote = 
...
install_sh = ${SHELL} /home/paolo/Src/ezinstall-remote/install-sh
...
install-ezinstallcssDATA: $(ezinstallcss_DATA)
	...
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(ezinstallcssdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(ezinstallcssdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(ezinstallcssdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(ezinstallcssdir)" || exit $$?; \
	done

uninstall-ezinstallcssDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(ezinstallcss_DATA)'; test -n "$(ezinstallcssdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(ezinstallcssdir)'; $(am__uninstall_files_from_dir)
install-ezinstalldocDATA: $(ezinstalldoc_DATA)
	@$(NORMAL_INSTALL)
	@list='$(ezinstalldoc_DATA)'; test -n "$(ezinstalldocdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(ezinstalldocdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(ezinstalldocdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(ezinstalldocdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(ezinstalldocdir)" || exit $$?; \
	done
...
uninstall-ezinstalldocDATA:
	...
	@list='$(ezinstalldoc_DATA)'; test -n "$(ezinstalldocdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(ezinstalldocdir)'; $(am__uninstall_files_from_dir)
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	install-exec install-exec-am install-ezinstallcfgDATA \
	...
	tags-am uninstall uninstall-am uninstall-ezinstallcfgDATA \
